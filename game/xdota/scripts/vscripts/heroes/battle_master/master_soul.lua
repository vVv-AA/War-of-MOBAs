LinkLuaModifier("modifier_master_soul", "heroes/battle_master/modifier_master_soul.lua", LUA_MODIFIER_MOTION_NONE)

function AddMod( event )
	local caster = event.caster
	local ability = event.ability

	caster:AddNewModifier(caster, ability, "modifier_master_soul", {duration = ability:GetLevelSpecialValueFor("duration", ability:GetLevel() - 1)})
end
