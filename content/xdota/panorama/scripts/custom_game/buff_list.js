...
function UpdateBuff( buffPanel, queryUnit, buffSerial )
{
	...
	var duration = Buffs.GetRemainingTime(queryUnit, buffSerial);
	if(duration > 0) {
		buffPanel.FindChildInLayoutFile( "Duration" ).text = new String(duration.toFixed(1));
	}
	else {
		buffPanel.FindChildInLayoutFile( "Duration" ).text = "";
	}
}
...
function UpdateBuffs()
{
	...
	for ( var i = 0; i < nBuffs; ++i )
	{
		...
		UpdateBuff( m_BuffPanels[ nUsedPanels++ ], queryUnit, buffSerial );
	}

	// clear any remaining panels
	for ( var i = nUsedPanels; i < m_BuffPanels.length; ++i )
	{
		UpdateBuff( m_BuffPanels[ i ], -1, -1 );
	}
}
